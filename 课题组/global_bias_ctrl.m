function [sys,x0,str,ts] = global_bias_ctrl(t,x,u,flag)
...
global node gamma1 gamma2 delta b K1 K2 center p Td g bg
bg=[4,4]; % 全局偏置
%负载
Td=[0,0]';
...
sys = simsizes(sizes);
x0  = zeros(1,(node+1)*2);%初始的神经网络权值
str = [];
ts  = [];

function sys=mdlDerivatives(t,x,u)
global node gamma1 gamma2 delta b K1 center bg
xd1=u(1);
d_xd1=u(2);
dd_xd1=u(3);
xd2=u(4);
...
W1=x(1:node+1)';
norm_W1=norm(W1);
W2=x((node+1)+1:2*(node+1))';
norm_W2=norm(W2);
%限制范数大小
if(norm_W1>14)
    sys(1:node+1)=gamma1*(SG(1,:)*de(1)-delta*W1);
    
else
     sys(1:node+1)=gamma1*SG(1,:)*r(1);  
end

if(norm_W2>14)
   sys((node+1)+1:2*(node+1))=gamma2*(SG(2,:)*de(2)-delta*W2);
    
else
     sys((node+1)+1:2*(node+1))=gamma2*SG(2,:)*r(2);  
end
...
global node K2 b K1 center  Td p g bg
xd1=u(1);
d_xd1=u(2);
dd_xd1=u(3);
xd2=u(4);
