...
touchX = SerialLink([L(1),L(2),L(3),L(4)],'name','touchX01');
%teach(touchX); 示教部分可选
% touchX.plot([0,0,0,0,0,0]);
% hold on;

%6个关节角的限制设置   这个是参考网上文件的
theta1min=-0.9;theta1max=0.9;
theta2min=0;theta2max=1.73;
theta3min=-0.9;theta3max=1.2;
theta4min=-2.2;theta4max=2.4;
theta5min=-0.45;theta5max=1.7;
theta6min=-2.4;theta6max=2.4;
...
% theta5min=-0.5;theta5max=2.286; %6.24用的它试试
% % theta5min=-0.5;theta5max=1.75;%测量的范围为-0.218~2.286
...
%     touchX.plot([q1,q2,q3,q4]);%动画显示,可不用
%     touchX.plot([q1,q2,q3,q4,q5,q6]);%动画显示,可不用
    trans = touchX.fkine([q1,q2,q3,q4]);%求解
    %trans中有四个向量t，n，o，a
